...
func executeTests(funkerName string, testChunks [][]string) error {
	...
	for chunkID, tests := range testChunks {
		...
		go func(chunkID int, tests []string) {
			defer wg.Done()
			log.Printf("Executing chunk %d (contains %d tests)", chunkID, len(tests))
			chunkBegin := time.Now()
			...
			if err != nil {
				...
				log.Printf("Finished chunk %d [%d/%d] in %s, code=%d.",
					chunkID, passed+failed, len(testChunks), time.Now().Sub(chunkBegin), result.Code)
			}
		}(chunkID, tests)
	}
	wg.Wait()
