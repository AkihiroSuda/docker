...
run_master_container(){
    # TODO: we should better use bash4 dictionary object?
    chunks="$1" network="$2" worker="$3" name="$4" image="$5" volume="$6" shuffle="$7" random_chunking="$8" rand_seed="$9"
    ...
    docker container run -it --rm \
	   ...
	   -input=/mnt/input \
	   -shuffle=$shuffle \
	   -random-chunking=$random_chunking \
	   -rand-seed=$rand_seed
    code=$?
    set +x
    return $code
...
main() {
	...
	case "$1" in
		...
		;;
	    *)
		errexit "Usage: $0 --replicas N --chunks N --push-worker-image NAME --shuffle --random-chunking --rand-seed N --filters-file NAME --dry-run"
		;;
	esac
    done
    ...
    # Start the master
    set +e
    run_master_container $chunks $network $worker_service $master_container $master_image $volume $shuffle $random_chunking $rand_seed
