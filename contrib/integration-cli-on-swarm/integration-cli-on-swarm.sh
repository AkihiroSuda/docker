...
main() {
    ...
    # Follow service logs in background
    docker service logs --follow $worker_service &
    worker_service_logs_pid=$!

    # Register cleaner for ^C
    trap "kill -9 $worker_service_logs_pid > /dev/null 2>&1; cleanup $network $master_container $worker_service || true" INT

    # Follow master logs in foreground
