...
func xmain() error {
	...
	if err = createVolumeWithData(cli, defaultVolumeName, map[string][]byte{"/input": filters}); err != nil {
		return err
	}
...
func ensureImages(cli *client.Client, images []string) error {
	for _, image := range images {
		...
		if err != nil {
			return fmt.Errorf("could not find image %s (%v). please run `make build-integration-cli-on-swarm`.",
				image, err)
		}
	}
	return nil
