...
func (cli *DaemonCli) start(opts *daemonOptions) (err error) {
	...
	if runtime.GOOS == "linux" && os.Geteuid() != 0 {
		return fmt.Errorf("rootless mode requires userns, mountns, and netns to be unshared before running the daemon; you may use RootlessKit and slirp4netns (or VPNKit) for setting up them")
	}
	if cli.Config.Rootless {
		logrus.Warn("Running in rootless mode (experimental). Cgroups, AppArmor, SELinux, and CRIU are not likely to work.")
		if !cli.Config.Experimental {
