...
func TestContainerBindMountNoRecursive(t *testing.T) {
	skip.If(t, testEnv.DaemonInfo.OSType != "linux" || testEnv.IsRemoteDaemon())
	skip.If(t, versions.LessThan(testEnv.DaemonAPIVersion(), "1.40"), "BindOptions.NoRecursive requires API v1.40")
	...
	tmpDir2 := fs.NewDir(t, "tmpdir2", fs.WithMode(0755),
		fs.WithFile("file", "should not be visible when NoRecursive", fs.WithMode(0644)))
	...
	// implicit is recursive (NoRecursive: false)
