...
// rawTmpfsOptions converts *mounttypes.TmpfsOptions to the raw option string
// for mount(2).
...
func rawTmpfsOptions(opt *mounttypes.TmpfsOptions) (string, error) {
	return rawTmpfsOptionsForOS(opt, runtime.GOOS)
}

func rawTmpfsOptionsForOS(opt *mounttypes.TmpfsOptions, os string) (string, error) {
	if opt == nil {
		return "", nil
	}
	var rawOpts []string
	...
	if opt.GID != 0 {
		rawOpts = append(rawOpts, fmt.Sprintf("gid=%d", opt.GID))
	}
	if opt.Blocks != 0 {
		if os != "linux" {
			return "", fmt.Errorf("Blocks is not supported on %s", os)
		}
		if opt.SizeBytes != 0 {
			return "", fmt.Errorf("exclusive: Blocks vs SizeBytes")
		}
		rawOpts = append(rawOpts, fmt.Sprintf("nr_blocks=%d", opt.Blocks))
	}
	if opt.INodes != 0 {
		switch os {
		case "linux":
			rawOpts = append(rawOpts, fmt.Sprintf("nr_inodes=%d", opt.INodes))
		case "freebsd":
			rawOpts = append(rawOpts, fmt.Sprintf("inodes=%d", opt.INodes))
		default:
			return "", fmt.Errorf("Inodes is not supported on %s", os)
		}
	}
	if opt.MemoryPolicy != "" {
		if os != "linux" {
			return "", fmt.Errorf("MemoryPolicy is not supported on %s", os)
		}
		rawOpts = append(rawOpts, fmt.Sprintf("mpol=%s", opt.MemoryPolicy))
	}
