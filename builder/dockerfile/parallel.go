...
func newParallelBuilder(clientCtx context.Context, config *types.ImageBuildOptions, backend builder.Backend, buildContext builder.Context, dockerfile io.ReadCloser) (*parallelBuilder, error) {
	...
	if dockerfile != nil {
		...
		dockerfileB = []byte(config.Dockerfile)
	}
	...
	logrus.Infof("[PARALLEL BUILDER] DAG: %d stages, %d nodes, %d edges: %+v",
		len(b.stages), len(b.daggy.Nodes), len(b.daggy.Edges), b.daggy)
	return b, nil
}
...
func (b *parallelBuilder) build(stdout io.Writer, stderr io.Writer, out io.Writer) (string, error) {
	...
	logrus.Infof("[PARALLEL BUILDER] DAG Schedule: %+v", schedRoot)
	logrus.Warn("[PARALLEL BUILDER] parallelism is forcibly set to 0 (infinity)") // unimplemented
	var wg sync.WaitGroup
