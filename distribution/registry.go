...
func NewV2Repository(ctx context.Context, repoInfo *registry.RepositoryInfo, endpoint registry.APIEndpoint, metaHeaders http.Header, authConfig *types.AuthConfig, actions ...string) (repo distribution.Repository, foundVersion bool, err error) {
	...
	if endpoint.URL.Scheme == "unix" {
		...
		logrus.Debugf("Configuring UNIX transport for %q", path)
		// how to split url properly?
		...
		// We heva something similar in client.buildRequest as well.
		// TODO(AkihiroSuda): move the fake logic to go-connections.
		oldURL := *endpoint.URL
		endpoint.URL, _ = url.Parse("http://registry")
		logrus.Debugf("Faked %q into %q", oldURL, endpoint.URL)
	} else {
