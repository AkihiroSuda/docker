...
func (s *DockerSuite) TestRunIntrospection(c *check.C) {
	...
	out, _ := dockerCmd(c,
		...
		"-d",
		"--name", cName,
		"--mount", "type=introspection,target="+mount,
		"busybox", "top")
...
func (s *DockerSuite) TestRunIntrospectionError(c *check.C) {
	...
	cases := []testCase{
		{
			mountOpts:     []string{"--mount", "type=introspection"},
			expectedError: "target is required",
		},
		{
			mountOpts:     []string{"--mount", "type=introspection,target=/foo,readonly=false"},
			expectedError: "cannot set readonly=false explicitly",
		},
		{
			mountOpts:     []string{"--mount", "type=introspection,target=/foo,source=/bar"},
...
func (s *DockerSuite) TestRunIntrospectionNonExperimental(c *check.C) {
	...
	out, _, err := dockerCmdWithError("run", "-d", "--mount", "type=introspection,target=/foo", "busybox", "top")
	c.Assert(err, checker.NotNil)
